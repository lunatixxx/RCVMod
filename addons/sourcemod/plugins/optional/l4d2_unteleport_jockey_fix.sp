...
public OnPluginStart()
{
	...
	BuildPath(Path_SM, path, 256, "logs/teleportation.txt");
...
public Action Event_JockeyDeath(Event h_Event, const char[] s_Name, bool b_DontBroadcast)
{
	...
	g_iJockeyVictim = -1;
	#if debug
		CPrintToChatAll("{blue}[FIX]{default} Jockey killed.");
	#endif
...
void TeleportToPrevPos(int victim)
{
	if (IsClientInGame(victim) && GetClientTeam(victim) == 2 && IsPlayerAlive(victim))
	{
		...
		#if debug
			CPrintToChatAll("{blue}[Jockey UnTeleport]{default} trying to teleport survivor to his original position.");
		#endif
	}
}
